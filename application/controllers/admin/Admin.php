...
class Admin extends Admin_Controller{
  ...
  function index()
  {
    ...
    // load content
    $data['content']       = $this->load->view('admin/admin', $data, TRUE);
    // load template
    $data['title']         = "Admin";
      ...
      $this->form_validation->set_rules('phone', 'Phone', 'trim|required|xss_clean|numeric');
	    ...
	    if(!$this->form_validation->run())
	    {
	      $this->session->set_flashdata('error', validation_errors());
				redirect('admin/Admin/New', 'refresh');
	    }
      else
      {
  ...
  public function editAdmin($id)
  {
    if ($this->input->server('REQUEST_METHOD') == 'POST')
    {
      ...
      $this->form_validation->set_rules('phone', 'Phone', 'trim|required|xss_clean|numeric');
	    ...
	    {
	      $this->session->set_flashdata('error', validation_errors());
				redirect('admin/Admin/editAdmin/' . $id, 'refresh');
	    }
      else
      {
        $data['name']    = $this->input->post('name');
    ...
    // load page
    $data['content']        = $this->load->view('admin/admin_edit', $data, TRUE);
